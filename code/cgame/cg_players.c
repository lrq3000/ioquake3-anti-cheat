		       ...
		       const char *ext)
{
  ...
  while (1)
  {
	...
	Com_sprintf(filename, length,
		    ...
		    charactersFolder, modelName, teamName, base,
		    skinName, team, ext);
      }
      else
      {
...
CG_FindClientHeadFile(char *filename, int length, clientInfo_t * ci,
		      ...
		      const char *headSkinName, const char *base,
		      const char *ext)
{
  ...
  while (1)
  {
    for (i = 0; i < 2; i++)
    {
      ...
      {
	Com_sprintf(filename, length,
		    "models/players/%s%s/%s/%s%s_%s.%s", headsFolder,
		    headModelName, headSkinName, teamName, base, team, ext);
      }
      else
      {
	Com_sprintf(filename, length,
		    "models/players/%s%s/%s/%s_%s.%s", headsFolder,
	...
	{
	  Com_sprintf(filename, length,
		      "models/players/%s%s/%s_%s.%s", headsFolder,
		      ...
		      const char *headModelName, const char *headSkinName)
{
  ...
  if (CG_FindClientModelFile
      (filename, sizeof(filename), ci, teamName, modelName, skinName,
       "lower", "skin"))
  {
    ci->legsSkin = trap_R_RegisterSkin(filename);
  }
  ...
  if (CG_FindClientHeadFile
      (filename, sizeof(filename), ci, teamName, headModelName, headSkinName,
       "head", "skin"))
  {
			   ...
			   const char *headSkinName, const char *teamName)
{
  ...
  if (!CG_RegisterClientSkin
      (ci, teamName, modelName, skinName, headName, headSkinName))
  {
    if (teamName && *teamName)
    {
	...
	Com_sprintf(newTeamName, sizeof(newTeamName), "%s/",
      ...
      if (!CG_RegisterClientSkin
	  (ci, newTeamName, modelName, skinName, headName, headSkinName))
      {
	Com_Printf("Failed to load skin file: %s : %s : %s, %s : %s\n",
  ...
  if (CG_FindClientHeadFile
      (filename, sizeof(filename), ci, teamName, headName, headSkinName,
       "icon", "skin"))
  {
...
CG_LoadClientInfo(int clientNum, clientInfo_t * ci)
{
  ...
  if (!CG_RegisterClientModelname
      (ci, ci->modelName, ci->skinName, ci->headModelName, ci->headSkinName,
       teamname))
  {
	...
	Q_strncpyz(teamname, DEFAULT_REDTEAM_NAME, sizeof(teamname));
      ...
      if (!CG_RegisterClientModelname
	  (ci, DEFAULT_TEAM_MODEL, ci->skinName, DEFAULT_TEAM_HEAD,
	   ci->skinName, teamname))
      {
	CG_Error
	  ("DEFAULT_TEAM_MODEL / skin (%s/%s) failed to register",
    ...
    {
      if (!CG_RegisterClientModelname
	  (ci, DEFAULT_MODEL, "default", DEFAULT_MODEL, "default", teamname))
      {
  ...
  for (i = 0; i < MAX_CUSTOM_SOUNDS; i++)
  {
    ...
    {
      ci->sounds[i] =
	trap_S_RegisterSound(va("sound/player/%s/%s", fallback, s + 1),
			     qfalse);
    }
  }
...
CG_NewClientInfo(int clientNum)
{
  ...
  if (cg_forceModel.integer)
  {
    ...
    if (cgs.gametype >= GT_TEAM)
    {
      Q_strncpyz(newInfo.headModelName, DEFAULT_TEAM_HEAD,
  ...
  if (!CG_ScanForExistingClientInfo(&newInfo))
  {
    ...
    if (forceDefer
	|| (cg_deferPlayers.integer && !cg_buildScript.integer
	    && !cg.loading))
    {
...
*/
static void
CG_SetLerpFrameAnimation(clientInfo_t * ci, lerpFrame_t * lf,
			 int newAnimation)
{
  animation_t *anim;
...
*/
static void
CG_PlayerAnimation(centity_t * cent, int *legsOld, int *legs,
...
*/
static void
CG_PlayerAngles(centity_t * cent, vec3_t legs[3], vec3_t torso[3],
		vec3_t head[3])
{
...
CG_PlayerTokens(centity_t * cent, int renderfx)
{
  ...
  for (i = 0; i < trail->numpositions; i++)
  {
    ...
    angle =
      (((cg.time + 500 * MAX_SKULLTRAIL -
	 500 * i) / 16) & 255) * (M_PI * 2) / 255;
...
CG_PlayerPowerups(centity_t * cent, refEntity_t * torso)
{
  ...
  {
    trap_R_AddLightToScene(cent->lerpOrigin, 200 + (rand() & 31), 0.2f,
			   0.2f, 1);
  }
  ...
  if (powerups & (1 << PW_REDFLAG))
  {
    ...
    trap_R_AddLightToScene(cent->lerpOrigin, 200 + (rand() & 31), 1.0,
			   0.2f, 0.2f);
  }
