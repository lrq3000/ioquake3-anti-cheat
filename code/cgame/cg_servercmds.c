  ...
  if (cg_singlePlayerActive.integer)
  {
	...
	&& *cg_recordSPDemoName.string)
    {
      trap_SendConsoleCommand(va
			      ("set g_synchronousclients 1 ; record %s \n",
			       cg_recordSPDemoName.string));
    }
  }
#endif
  ...
  while (1)
  {
	...
	return qtrue;
      ...
      Com_sprintf(voiceChats[voiceChatList->numVoiceChats].
		  chats[voiceChats[voiceChatList->numVoiceChats].
...
CG_LoadVoiceChats(void)
{
  ...
  CG_ParseVoiceChats("scripts/female3.voice", &voiceChatLists[2],
		     MAX_VOICECHATS);
  CG_ParseVoiceChats("scripts/male1.voice", &voiceChatLists[3],
		     MAX_VOICECHATS);
  CG_ParseVoiceChats("scripts/male2.voice", &voiceChatLists[4],
		     MAX_VOICECHATS);
  CG_ParseVoiceChats("scripts/male3.voice", &voiceChatLists[5],
		     MAX_VOICECHATS);
  CG_ParseVoiceChats("scripts/male4.voice", &voiceChatLists[6],
		     MAX_VOICECHATS);
  CG_ParseVoiceChats("scripts/male5.voice", &voiceChatLists[7],
		     MAX_VOICECHATS);
...
*/
int
CG_GetVoiceChat(voiceChatList_t * voiceChatList, const char *id,
...
CG_VoiceChatListForClient(int clientNum)
{
  ...
  for (k = 0; k < 2; k++)
  {
    ...
    for (i = 0; i < MAX_HEADMODELS; i++)
    {
      ...
      {
	Com_sprintf(filename, sizeof(filename), "scripts/%s.vc",
		    headModelName);
	  ...
	  for (j = 0; j < MAX_HEADMODELS; j++)
	  {
	    if (!strlen(headModelVoiceChat[j].headmodel))
	    {
	      Com_sprintf(headModelVoiceChat[j].headmodel,
			  sizeof(headModelVoiceChat[j].
				 headmodel), "%s", headModelName);
...
CG_PlayBufferedVoiceChats(void)
{
  ...
  if (cg.voiceChatTime < cg.time)
  {
	...
	(cg.voiceChatBufferOut + 1) % MAX_VOICECHATBUFFER;
...
CG_VoiceChat(int mode)
{
  ...
  {
    if (!strcmp(cmd, VOICECHAT_KILLINSULT)
...
CG_ServerCommand(void)
{
  ...
  if (!strcmp(cmd, "print"))
  {
	...
	|| !Q_stricmpn(cmd, "team vote failed", 16))
    {
      trap_S_StartLocalSound(cgs.media.voteFailed, CHAN_ANNOUNCER);
    }
    else if (!Q_stricmpn(cmd, "vote passed", 11)
	     || !Q_stricmpn(cmd, "team vote passed", 16))
    {
