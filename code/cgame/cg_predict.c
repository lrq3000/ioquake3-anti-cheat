...
*/
static void
CG_ClipMoveToEntities(const vec3_t start, const vec3_t mins,
...
CG_PointContents(const vec3_t point, int passEntityNum)
{
  ...
  for (i = 0; i < cg_numSolidEntities; i++)
  {
    ...
    contents |=
      trap_CM_TransformedPointContents(point, cmodel, cent->lerpOrigin,
				       cent->lerpAngles);
  }
...
CG_InterpolatePlayerState(qboolean grabAngles)
{
  ...
  *out = cg.snap->ps;
  ...
  f =
    (float) (cg.time - prev->serverTime) / (next->serverTime -
					    prev->serverTime);
...
CG_TouchItem(centity_t * cent)
{
  ...
  if (!BG_PlayerTouchesItem
      (&cg.predictedPlayerState, &cent->currentState, cg.time))
  {
    return;
  }
...
CG_PredictPlayerState(void)
{
  ...
  for (cmdNum = current - CMD_BACKUP + 1; cmdNum <= current; cmdNum++)
  {
	...
	CG_AdjustPositionForMover(cg.predictedPlayerState.origin,
				  cg.predictedPlayerState.
				  groundEntityNum, cg.physicsTime,
				  cg.oldTime, adjusted,
