...
*/
void
CG_DrawHead(float x, float y, float w, float h, int clientNum,
	    vec3_t headAngles)
{
  clipHandle_t cm;
  clientInfo_t *ci;
...
*/
void
CG_DrawFlagModel(float x, float y, float w, float h, int team,
		 qboolean force2D)
{
  qhandle_t cm;
  float len;
...
//              { 0.2, 1.0, 0.2, 1.0 } , { 1.0, 0.2, 0.2, 1.0 }, {0.5, 0.5, 0.5, 1} };
  ...
  {
    CG_DrawStatusBarFlag(185 + CHAR_WIDTH * 3 + TEXT_ICON_SPACE +
			 ICON_SIZE, TEAM_RED);
  }
  ...
  if (cent->currentState.weapon)
  {
	...
	if (icon)
	{
	  CG_DrawPic(CHAR_WIDTH * 3 + TEXT_ICON_SPACE, 432,
...
//                                      ((lwidth/2 - len/2) * TINYCHAR_WIDTH);
	xx = x + TINYCHAR_WIDTH * 2 + TINYCHAR_WIDTH * pwidth;
	CG_DrawStringExt(xx, y,
			 p, hcolor, qfalse, qfalse, TINYCHAR_WIDTH,
...
#ifdef MISSIONPACK
  s =
    va("VOTE(%i):%s yes:%i no:%i", sec, cgs.voteString, cgs.voteYes,
       cgs.voteNo);
