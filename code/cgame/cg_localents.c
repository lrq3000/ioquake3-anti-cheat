...
CG_AddMoveScaleFade(localEntity_t * le)
{
  ...
  if (le->fadeInTime > le->startTime && cg.time < le->fadeInTime)
  {
    // fade / grow time
    c =
      1.0 - (float) (le->fadeInTime - cg.time) / (le->fadeInTime -
						  le->startTime);
  }
  else
  {
...
//                      trap_S_StartSound (re->origin, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.kamikazeExplodeSound );
    ...
    c =
      (float) (t -
	       KAMI_SHOCKWAVE_STARTTIME) / (float) (KAMI_SHOCKWAVE_ENDTIME -
...
//                              trap_S_StartSound (re->origin, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.kamikazeImplodeSound );
	...
	(float) (KAMI_IMPLODE_ENDTIME -
		 t) / (float) (KAMI_IMPLODE_ENDTIME - KAMI_IMPLODE_STARTTIME);
    }
    VectorScale(axis[0],
		c * KAMI_BOOMSPHERE_MAXRADIUS /
  ...
  if (t > KAMI_SHOCKWAVE2_STARTTIME && t < KAMI_SHOCKWAVE2_ENDTIME)
  {
	...
	le->angles.trBase[1] == 0 && le->angles.trBase[2] == 0)
    {
...
CG_AddScorePlum(localEntity_t * le)
{
  ...
  {
    VectorMA(origin, (float) (((float) numdigits / 2) - i) * NUMBER_SIZE,
	     vec, re->origin);
