...
BotSetTeamStatus(bot_state_t * bs)
{
  ...
  case LTG_TEAMACCOMPANY:
    ...
    if (((gametype == GT_CTF || gametype == GT_1FCTF)
	 && EntityCarriesFlag(&entinfo)) || (gametype == GT_HARVESTER
...
BotSetLastOrderedTask(bot_state_t * bs)
{
  ...
  if (bs->lastgoal_ltgtype)
  {
	...
	eb = BotEnemyFlag(bs);
	tt =
	  trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
...
BotCanAndWantsToRocketJump(bot_state_t * bs)
{
  ...
  rocketjumper =
    trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_WEAPONJUMPING,
			       0, 1);
  if (rocketjumper < 0.5)
    return qfalse;
  return qtrue;
...
BotAttackMove(bot_state_t * bs, int tfl)
{
  ...
  //
  attack_skill =
    trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_ATTACK_SKILL, 0,
			       1);
  jumper =
...
*/
float
BotEntityVisible(int viewer, vec3_t eye, vec3_t viewangles, float fov,
		 int ent)
{
...
BotFindEnemy(bot_state_t * bs, int curenemy)
{
  ...
  easyfragger =
    trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_EASY_FRAGGER, 0,
			       1);
  ...
  for (i = 0; i < maxclients && i < MAX_CLIENTS; i++)
  {
	...
	90 + 90 - (90 -
		   (squaredist >
		    Square(810) ? Square(810) : squaredist) / (810 * 9));
...
BotAimAtEnemy(bot_state_t * bs)
{
  ...
  aim_accuracy =
    trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY, 0,
			       1);
  //
  if (aim_skill > 0.95)
  {
  ...
  if (wi.number == WP_MACHINEGUN)
  {
    aim_accuracy =
      trap_Characteristic_BFloat(bs->character,
				 CHARACTERISTIC_AIM_ACCURACY_MACHINEGUN, 0,
				 1);
  }
	  ...
	  if (VectorLengthSquared(dir) < Square(60))
	  {
	    ...
	    if (VectorLengthSquared(dir) > Square(100))
	    {
	      ...
	      BotAI_Trace(&trace, trace.endpos, NULL, NULL,
			  entinfo.origin, entinfo.number, MASK_SHOT);
	      if (trace.fraction >= 1)
	      {
  ...
  else
  {
    ...
    if (aim_skill > 0.5)
    {
      ...
      if (wi.number == WP_BFG ||
	  wi.number == WP_ROCKET_LAUNCHER || wi.number == WP_GRENADE_LAUNCHER)
      {
	//create the chase goal
	...
	if (trap_BotPredictVisiblePosition
	    (bs->lastenemyorigin, bs->lastenemyareanum, &goal,
	     TFL_DEFAULT, target))
	{
	  VectorSubtract(target, bs->eye, dir);
...
BotCheckAttack(bot_state_t * bs)
{
  ...
  //
  reactiontime =
    trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_REACTIONTIME, 0,
			       1);
  if (bs->enemysight_time > FloatTime() - reactiontime)
    return;
  if (bs->teleport_time > FloatTime() - reactiontime)
  ...
  if (bs->firethrottlewait_time > FloatTime())
    return;
  firethrottle =
    trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_FIRETHROTTLE, 0,
			       1);
			  ...
			  bot_activategoal_t * activategoal)
{
  ...
  dist =
    fabs(movedir[0]) * size[0] + fabs(movedir[1]) * size[1] +
...
*/
int
BotPushOntoActivateGoalStack(bot_state_t * bs,
			     bot_activategoal_t * activategoal)
{
  int i, best;
  float besttime;
...
int
BotGetActivateGoal(bot_state_t * bs, int entitynum,
		   bot_activategoal_t * activategoal)
{
       ...
       ent = trap_AAS_NextBSPEntity(ent))
  {
	...
	(ent, "model", tmpmodel, sizeof(tmpmodel)))
      continue;
  ...
  if (!trap_AAS_ValueForBSPEpairKey
      (ent, "targetname", targetname[0], sizeof(targetname[0])))
  {
  ...
  for (i = 0; i >= 0 && i < 10;)
  {
    ...
    {
      if (!trap_AAS_ValueForBSPEpairKey
	  (ent, "target", target, sizeof(target)))
	continue;
    ...
    if (!trap_AAS_ValueForBSPEpairKey
	(ent, "classname", classname, sizeof(classname)))
    {
      if (bot_developer.integer)
      {
	BotAI_Print(PRT_ERROR,
    ...
    {
      if (trap_AAS_ValueForBSPEpairKey
	  (ent, "targetname", targetname[i + 1], sizeof(targetname[0])))
      {
	i++;
...
BotAIPredictObstacles(bot_state_t * bs, bot_goal_t * goal)
{
  ...
  if (route.stopevent & RSE_ENTERCONTENTS)
  {
    ...
    if (route.endcontents & AREACONTENTS_MOVER)
    {
      ...
      modelnum =
	(route.
	 endcontents & AREACONTENTS_MODELNUM) >> AREACONTENTS_MODELNUMSHIFT;
      if (modelnum)
      {
	//
  ...
  while ((handle = trap_BotNextConsoleMessage(bs->cs, &m)) != 0)
  {
	...
	if (bot_testrchat.integer)
	{
	  ...
	  if (trap_BotReplyChat
	      (bs->cs, message, context, CONTEXT_REPLY, NULL, NULL,
		 ...
		 && !TeamPlayIsOn())
	{
	  ...
	  if (random() < 1.5 / (NumBots() + 1) && random() < chat_reply)
	  {
	    //if bot replies with a chat message
	    if (trap_BotReplyChat
		(bs->cs, message, context, CONTEXT_REPLY, NULL,
		 NULL, NULL, NULL, NULL, NULL, botname, netname))
	    {
...
BotCheckEvents(bot_state_t * bs, entityState_t * state)
{
  ...
  case EV_OBITUARY:
	...
	//
	if (target == attacker ||
	    target == ENTITYNUM_NONE || target == ENTITYNUM_WORLD)
	  bs->botsuicide = qtrue;
	else
...
BotSetupAlternativeRouteGoals(void)
{
  ...
  if (gametype == GT_CTF)
  {
    ...
    {
      //
      red_numaltroutegoals =
	trap_AAS_AlternativeRouteGoals(ctf_neutralflag.origin,
  ...
  else if (gametype == GT_OBELISK)
  {
    ...
    //
    red_numaltroutegoals =
      trap_AAS_AlternativeRouteGoals(neutralobelisk.origin,
				     neutralobelisk.areanum,
				     redobelisk.origin, redobelisk.areanum,
				     TFL_DEFAULT, red_altroutegoals,
				     MAX_ALTROUTEGOALS,
				     ALTROUTEGOAL_CLUSTERPORTALS |
				     ALTROUTEGOAL_VIEWPORTALS);
    blue_numaltroutegoals =
