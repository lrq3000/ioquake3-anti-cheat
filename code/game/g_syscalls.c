...
void
trap_Cvar_VariableStringBuffer(const char *var_name, char *buffer,
			       int bufsize)
{
...
int
trap_EntitiesInBox(const vec3_t mins, const vec3_t maxs, int *list,
		   int maxcount)
{
...
qboolean
trap_EntityContact(const vec3_t mins, const vec3_t maxs,
		   const gentity_t * ent)
{
...
int
trap_AAS_AreaTravelTimeToGoalArea(int areanum, vec3_t origin,
...
int
trap_AAS_PredictRoute(void /*struct aas_predictroute_s */ *route,
...
int
trap_AAS_AlternativeRouteGoals(vec3_t start, int startareanum, vec3_t goal, int goalareanum, int travelflags, void	/*struct aas_altroutegoal_s */
			       *altroutegoals, int maxaltroutegoals, int type)
{
  return syscall(BOTLIB_AAS_ALTERNATIVE_ROUTE_GOAL, start, startareanum,
		 goal, goalareanum, travelflags, altroutegoals,
...
trap_AAS_PredictClientMovement(void /* struct aas_clientmove_s */ *move,
			       ...
			       int onground, vec3_t velocity, vec3_t cmdmove,
...
int
trap_GeneticParentsAndChildSelection(int numranks, float *ranks,
