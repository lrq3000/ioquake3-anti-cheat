...
// g_spawn.c
//
qboolean G_SpawnString(const char *key, const char *defaultString,
		       char **out);
...
void trap_AAS_EntityInfo(int entnum,
			 void /* struct aas_entityinfo_s */ *info);
...
int trap_AAS_AlternativeRouteGoals(vec3_t start, int startareanum, vec3_t goal, int goalareanum, int travelflags, void	/*struct aas_altroutegoal_s */
...
void trap_Characteristic_String(int character, int index, char *buf,
				int size);
...
void trap_BotMatchVariable(void /* struct bot_match_s */ *match,
			   int variable, char *buf, int size);
