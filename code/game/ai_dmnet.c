  ...
  if (bs->lastair_time < FloatTime() - 6)
  {
    ...
    else
    {
      ...
      while (trap_BotChooseNBGItem
	     (bs->gs, bs->origin, bs->inventory, tfl, ltg, range))
      {
...
BotReachedGoal(bot_state_t * bs, bot_goal_t * goal)
{
  if (goal->flags & GFL_ITEM)
  {
	...
	(bs->entitynum, bs->eye, bs->viewangles, goal))
    {
      /*
         float avoidtime;
         int t;
...
BotGetLongTermGoal(bot_state_t * bs, int tfl, int retreat, bot_goal_t * goal)
{
  ...
  if (bs->ltgtype == LTG_TEAMHELP && !retreat)
  {
    ...
    {
      BotAI_BotInitialChat(bs, "help_start",
    ...
    if (BotEntityVisible
	(bs->entitynum, bs->eye, bs->viewangles, 360, bs->teammate))
    {
...
BotLongTermGoal(bot_state_t * bs, int tfl, int retreat, bot_goal_t * goal)
{
  ...
  if (bs->lead_time > 0 && !retreat)
  {
    ...
    //if the team mate is visible
    if (BotEntityVisible
	(bs->entitynum, bs->eye, bs->viewangles, 360, bs->lead_teammate))
    {
...
BotClearPath(bot_state_t * bs, bot_moveresult_t * moveresult)
{
  ...
  if (bs->kamikazebody)
  {
    ...
    if (!
	(moveresult->
	 flags & (MOVERESULT_MOVEMENTVIEW | MOVERESULT_MOVEMENTWEAPON)))
    {
      //
	...
	if (bs->cur_ps.weapon == moveresult->weapon)
	{
	  ...
	  if (InFieldOfVision
	      (bs->viewangles, 20, moveresult->ideal_viewangles))
	  {
	    //
	    BotAI_Trace(&bsptrace, bs->eye, NULL, NULL, target,
...
AINode_Seek_ActivateEntity(bot_state_t * bs)
{
  ...
  if (moveresult.
      flags & (MOVERESULT_MOVEMENTVIEWSET | MOVERESULT_MOVEMENTVIEW |
	       MOVERESULT_SWIMVIEW))
  {
...
AINode_Battle_Fight(bot_state_t * bs)
{
  ...
  if (!BotEntityVisible
      (bs->entitynum, bs->eye, bs->viewangles, 360, bs->enemy))
  {
#ifdef MISSIONPACK
    if (bs->enemy == redobelisk.entitynum
	|| bs->enemy == blueobelisk.entitynum)
