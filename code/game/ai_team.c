...
BotSayTeamOrderAlways(bot_state_t * bs, int toclient)
{
  ...
  if (bs->client == toclient)
  {
    ...
    Com_sprintf(teamchat, sizeof(teamchat), EC "(%s" EC ")" EC ": %s",
		name, buf);
	...
	for (i = 0; i < defenders; i++)
	{
	  ...
	  else
	  {
	    BotAI_BotInitialChat(bs, "cmd_accompany", name,
				 carriername, NULL);
	    BotSayVoiceTeamOrder(bs, teammates[i],
  ...
  if (!(bs->ctfstrategy & CTFS_AGRESSIVE))
  {
	...
	if (bs->flagcarrier != -1)
	{
	  ...
	  for (i = 0; i < attackers; i++)
	  {
	    ...
	    {
	      BotAI_BotInitialChat(bs, "cmd_accompanyme", name, NULL);
	      BotSayVoiceTeamOrder(bs,
				   teammates[numteammates - i - 1],
				   VOICECHAT_FOLLOWME);
	    }
	    else
	    {
...
BotTeamAI(bot_state_t * bs)
{
  ...
  case GT_CTF:
	...
	if (random() < 0.4)
	{
      ...
      //if it's time to give orders
      if (bs->teamgiveorders_time
	  && bs->teamgiveorders_time < FloatTime() - 3)
      {
	BotCTFOrders(bs);
	//
  ...
  case GT_1FCTF:
	...
	if (random() < 0.4)
	{
      ...
      //if it's time to give orders
      if (bs->teamgiveorders_time
	  && bs->teamgiveorders_time < FloatTime() - 2)
      {
	Bot1FCTFOrders(bs);
	//
	bs->teamgiveorders_time = 0;
