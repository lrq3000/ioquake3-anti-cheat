...
*/
void
BotAI_Trace(bsp_trace_t * bsptrace, vec3_t start, vec3_t mins, vec3_t maxs,
...
BotInterbreedBots(void)
{
  ...
  if (trap_GeneticParentsAndChildSelection
      (MAX_CLIENTS, ranks, &parent1, &parent2, &child))
  {
...
BotChangeViewAngles(bot_state_t * bs, float thinktime)
{
  ...
  {
    factor =
      trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_VIEW_FACTOR,
				 0.01f, 1);
    maxchange =
      trap_Characteristic_BFloat(bs->character,
				 CHARACTERISTIC_VIEW_MAXCHANGE, 1, 1800);
  }
  else
  {
    factor = 0.05f;
...
*/
void
BotInputToUserCommand(bot_input_t * bi, usercmd_t * ucmd,
...
BotWriteSessionData(bot_state_t * bs)
{
  ...
  s = va("%i %i %i %i %i %i %i %i"
	 ...
	 bs->lastgoal_teamgoal.maxs[1], bs->lastgoal_teamgoal.maxs[2]);
