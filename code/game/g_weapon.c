...
Bullet_Fire(gentity_t * ent, float spread, int damage, int mod)
{
  ...
  for (i = 0; i < 10; i++)
  {
	...
	if (G_InvulnerabilityEffect
	    (traceEnt, forward, tr.endpos, impactpoint, bouncedir))
	{
...
weapon_railgun_fire(gentity_t * ent)
{
  ...
  do
  {
    ...
    if (traceEnt->takedamage)
    {
      ...
      {
	if (G_InvulnerabilityEffect
	    (traceEnt, forward, trace.endpos, impactpoint, bouncedir))
	{
	  G_BounceProjectile(muzzle, impactpoint, bouncedir, end);
...
Weapon_HookThink(gentity_t * ent)
{
  if (ent->enemy)
  {
    ...
    v[0] =
      ent->enemy->r.currentOrigin[0] + (ent->enemy->r.mins[0] +
					ent->enemy->r.maxs[0]) * 0.5;
    v[1] =
      ent->enemy->r.currentOrigin[1] + (ent->enemy->r.mins[1] +
					ent->enemy->r.maxs[1]) * 0.5;
    v[2] =
      ent->enemy->r.currentOrigin[2] + (ent->enemy->r.mins[2] +
					ent->enemy->r.maxs[2]) * 0.5;
...
*/
static void
KamikazeShockWave(vec3_t origin, gentity_t * attacker, float damage,
