...
BotPrintTeamGoal(bot_state_t * bs)
{
  ...
  case LTG_GETFLAG:
    {
      BotAI_Print(PRT_MESSAGE,
		  "%s: I'm gonna get the flag for %1.0f secs\n", netname, t);
      break;
    }
  case LTG_RUSHBASE:
    {
  ...
  case LTG_GETITEM:
    {
      BotAI_Print(PRT_MESSAGE,
		  "%s: I'm gonna get an item for %1.0f secs\n", netname, t);
      break;
    }
  case LTG_KILL:
    {
      BotAI_Print(PRT_MESSAGE,
		  "%s: I'm gonna kill someone for %1.0f secs\n", netname, t);
      break;
    }
  case LTG_CAMP:
  case LTG_CAMPORDER:
...
BotAddressedToBot(bot_state_t * bs, bot_match_t * match)
{
  ...
  if (match->subtype & ST_ADDRESSED)
  {
    ...
    while (trap_BotFindMatch
	   (addressedto, &addresseematch, MTCONTEXT_ADDRESSEE))
    {
...
BotMatch_StopTeamLeaderShip(bot_state_t * bs, bot_match_t * match)
{
  ...
  {
    if (!Q_stricmp
	(bs->teamleader, ClientName(client, netname, sizeof(netname))))
    {
      bs->teamleader[0] = '\0';
  ...
  do
  {
    ...
    if (dist < bestdist)
    {
      ...
      BotAI_Trace(&trace, bs->eye, NULL, NULL, tmpgoal.origin,
		  bs->client, CONTENTS_SOLID | CONTENTS_PLAYERCLIP);
...
BotMatch_WhereAreYou(bot_state_t * bs, bot_match_t * match)
{
  ...
  if (bestitem != -1)
  {
	...
	trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin,
      ...
      {
	BotAI_BotInitialChat(bs, "teamlocation", nearbyitems[bestitem],
			     "red", NULL);
      }
...
BotMatchMessage(bot_state_t * bs, char *message)
{
  ...
  if (!trap_BotFindMatch(message, &match, MTCONTEXT_MISC
			 | MTCONTEXT_INITIALTEAMCHAT | MTCONTEXT_CTF))
  {
    return qfalse;
  }
