...
G_ExplodeMissile(gentity_t * ent)
{
  ...
  {
    if (G_RadiusDamage
	(ent->r.currentOrigin, ent->parent, ent->splashDamage,
...
*/
static void
ProximityMine_Die(gentity_t * ent, gentity_t * inflictor,
...
ProximityMine_ExplodeOnPlayer(gentity_t * mine)
{
  ...
  if (player->client->invulnerabilityTime > level.time)
  {
    G_Damage(player, mine->parent, mine->parent, vec3_origin,
	     mine->s.origin, 1000, DAMAGE_NO_KNOCKBACK, MOD_JUICED);
...
G_MissileImpact(gentity_t * ent, trace_t * trace)
{
  ...
  if (other->takedamage)
  {
	...
	if (G_InvulnerabilityEffect
	    (other, forward, ent->s.pos.trBase, impactpoint, bouncedir))
	{
  ...
  if (!strcmp(ent->classname, "hook"))
  {
    ...
    if (other->takedamage && other->client)
    {
      ...
      v[0] =
	other->r.currentOrigin[0] + (other->r.mins[0] +
				     other->r.maxs[0]) * 0.5;
      v[1] =
	other->r.currentOrigin[1] + (other->r.mins[1] +
				     other->r.maxs[1]) * 0.5;
      v[2] =
	other->r.currentOrigin[2] + (other->r.mins[2] +
				     other->r.maxs[2]) * 0.5;
  ...
  {
    if (G_RadiusDamage
	(trace->endpos, ent->parent, ent->splashDamage, ent->splashRadius,
...
gentity_t *
fire_nail(gentity_t * self, vec3_t start, vec3_t forward, vec3_t right,
	  vec3_t up)
{
  gentity_t *bolt;
  vec3_t dir;
