...
UpdateTournamentInfo(void)
{
  ...
  else
  {
    ...
    Com_sprintf(msg, sizeof(msg),
		...
		player->client->ps.persistant[PERS_SCORE], perfect, score1,
		score2, level.time,
...
static gentity_t *
SpawnModelOnVictoryPad(gentity_t * pad, vec3_t offset, gentity_t * ent,
		       int place)
{
  gentity_t *body;
  vec3_t vec;
...
SpawnModelsOnVictoryPads(void)
{
  ...
  player =
    SpawnModelOnVictoryPad(podium, offsetFirst,
  ...
  player =
    SpawnModelOnVictoryPad(podium, offsetSecond,
  ...
  {
    player =
      SpawnModelOnVictoryPad(podium, offsetThird,
			     &g_entities[level.sortedClients[2]],
