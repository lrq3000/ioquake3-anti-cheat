...
*/
static void
SV_EmitPacketEntities(clientSnapshot_t * from, clientSnapshot_t * to,
		      msg_t * msg)
{
  ...
  while (newindex < to->num_entities || oldindex < from_num_entities)
  {
    ...
    {
      newent =
	&svs.snapshotEntities[(to->first_entity + newindex) %
    ...
    {
      oldent =
	&svs.snapshotEntities[(from->first_entity + oldindex) %
			      svs.numSnapshotEntities];
...
SV_UpdateServerCommandsToClient(client_t * client, msg_t * msg)
{
       ...
       i++)
  {
    ...
    MSG_WriteString(msg,
		    client->
		    reliableCommands[i & (MAX_RELIABLE_COMMANDS - 1)]);
  }
