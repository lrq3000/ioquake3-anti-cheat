...
SV_GetChallenge(netadr_t from)
{
      ...
      && !Sys_IsLANAddress(from))
  {
    ...
    if (svs.authorizeAddress.type == NA_BAD)
    {
      ...
      if (NET_StringToAdr
	  (AUTHORIZE_SERVER_NAME, &svs.authorizeAddress, NA_IP))
      {
	svs.authorizeAddress.port = BigShort(PORT_AUTHORIZE);
	Com_Printf("%s resolved to %i.%i.%i.%i:%i\n",
		   AUTHORIZE_SERVER_NAME, svs.authorizeAddress.ip[0],
	...
	strcpy(game, fs->string);
      ...
      NET_OutOfBandPrint(NS_SERVER, svs.authorizeAddress,
			 ...
			 from.ip[2], from.ip[3], game, sv_strictAuth->string);
...
SV_NextDownload_f(client_t * cl)
{
  ...
  if (block == cl->downloadClientBlock)
  {
    ...
    if (cl->
	downloadBlockSize[cl->downloadClientBlock % MAX_DOWNLOAD_WINDOW] == 0)
    {
...
SV_WriteDownloadToClient(client_t * cl, msg_t * msg)
{
	 ...
	 MAX_DOWNLOAD_WINDOW && cl->downloadSize != cl->downloadCount)
  {
    ...
    cl->downloadBlockSize[curindex] =
      FS_Read(cl->downloadBlocks[curindex], MAX_DOWNLOAD_BLKSIZE,
	      cl->download);
...
SV_UserVoip(client_t * cl, msg_t * msg)
{
  ...
  for (i = 0, client = svs.clients; i < sv_maxclients->integer; i++, client++)
  {
    ...
    client->
      voipPacket[(client->queuedVoipIndex +
