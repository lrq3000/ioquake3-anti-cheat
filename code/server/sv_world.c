  ...
  for (i = 0; i < num_leafs; i++)
  {
	    ...
	    && sv.state == SS_LOADING)
	{
	  Com_DPrintf("Object %i touching 3 areas at %f %f %f\n",
		      gEnt->s.number,
		      gEnt->r.absmin[0], gEnt->r.absmin[1],
		      gEnt->r.absmin[2]);
	}
	ent->areanum2 = area;
      }
		...
		int contentmask, int capsule)
{
  ...
  CM_TransformedBoxTrace(trace, (float *) start, (float *) end,
			 (float *) mins, (float *) maxs, clipHandle,
...
SV_ClipMoveToEntities(moveclip_t * clip)
{
  ...
  for (i = 0; i < num; i++)
  {
    ...
    CM_TransformedBoxTrace(&trace, (float *) clip->start,
			   (float *) clip->end, (float *) clip->mins,
			   (float *) clip->maxs, clipHandle,
			   clip->contentmask, origin, angles, clip->capsule);
...
SV_PointContents(const vec3_t p, int passEntityNum)
{
  ...
  for (i = 0; i < num; i++)
  {
    ...
    c2 =
      CM_TransformedPointContents(p, clipHandle, hit->r.currentOrigin,
				  angles);
