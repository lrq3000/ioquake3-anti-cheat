...
static void
predict_clip_velocity(vec3_t in, vec3_t normal, vec3_t out)
{
  float backoff;
...
static int
predict_slide_move(sharedEntity_t * ent, float frametime, trajectory_t * tr, vec3_t result)
{
...
static void
predict_move(sharedEntity_t * ent, float frametime, trajectory_t * tr, vec3_t result)
{
  float stepSize;
...
static void
calc_viewpoint(playerState_t * ps, vec3_t org, vec3_t vp)
{
  VectorCopy(org, vp);
...
static int
player_in_fov(vec3_t viewangle, vec3_t ppos, vec3_t opos)
{
...
static void
copy_trajectory(trajectory_t * src, trajectory_t * dst)
{
...
int
is_visible(vec3_t start, vec3_t end)
{
  trace_t trace;
...
int
SV_CanSee(int player, int other)
{
