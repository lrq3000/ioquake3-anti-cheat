...
static const char s_ascii_encoding[] =
  "0123456789abcdef" "ghijklmnopqrstuv" "wxyzABCDEFGHIJKL" "MNOPQRSTUVWXYZ[]";
  ...
  if (!strlen(Cvar_VariableString("sv_leagueName")))
  {
    status = GRankNewGameAsync
      (s_server_context,
       ...
       NULL,
       GR_OPT_LEAGUENAME,
       (void *) (Cvar_VariableString("sv_leagueName")), GR_OPT_END);
  }
  else
  {
    status = GRankNewGameAsync
      (s_server_context, SV_RankNewGameCBF, NULL, GR_OPT_END);
  }
...
SV_RankEnd(void)
{
  ...
  status = GRankSendReportsAsync
    (s_server_context, 0, SV_RankSendReportsCBF, NULL, GR_OPT_END);
...
*/
void
SV_RankUserCreate(int index, char *username, char *password, char *email)
{
  GR_INIT init;
  GR_STATUS status;
  ...
  status = GRankUserCreateAsync
    (s_ranked_players[index].context,
     username,
     password,
     email, SV_RankUserCBF, (void *) &s_ranked_players[index], GR_OPT_END);
...
SV_RankUserLogin(int index, char *username, char *password)
{
  ...
  status = GRankUserLoginAsync
    (s_ranked_players[index].context,
     username,
     password, SV_RankUserCBF, (void *) &s_ranked_players[index], GR_OPT_END);
...
SV_RankUserValidate(int index, const char *player_id, const char *key,
		    int token_len, int rank, char *name)
{
  GR_INIT init;
  GR_STATUS status;
  qboolean rVal;
  ...
  if ((player_id != NULL) && (key != NULL))
  {
    ...
    {
      if ((i != index)
	  && (s_ranked_players[i].grank_status == QGR_STATUS_ACTIVE)
    ...
    status = GRankPlayerValidate(s_server_context,
				 ...
				 ranked_player->context, GR_OPT_END);
  }
  else
  {
...
SV_RankUserLogout(int index)
{
  ...
  status = GRankSendReportsAsync
    (s_ranked_players[index].context,
     0, SV_RankSendReportsCBF, (void *) &s_ranked_players[index], GR_OPT_END);
  ...
  else
  {
    ...
    cleanup_status = GRankCleanupAsync
      (s_ranked_players[index].context,
       0, SV_RankCleanupCBF, (void *) &s_ranked_players[index], GR_OPT_END);
...
*/
void
SV_RankReportInt(int index1, int index2, int key, int value, qboolean accum)
{
  GR_STATUS status;
  GR_CONTEXT context;
  uint64_t match;
...
//              key, value, accum );
  ...
  status = GRankReportInt
    (context, match, user1, user2, key, value, opt_accum, GR_OPT_END);
...
//              key, value );
  ...
  status = GRankReportStr
    (context, match, user1, user2, key, value, GR_OPT_END);
	  ...
	  if (s_ranked_players[i].context)
	  {
	    ...
	    cleanup_status = GRankCleanupAsync
	      (s_ranked_players[i].context,
	       0,
	       SV_RankCleanupCBF,
	       (void *) &(s_ranked_players[i]), GR_OPT_END);
...
SV_RankUserCBF(GR_LOGIN * gr_login, void *cbf_arg)
{
  ...
  case GR_STATUS_OK:
    ...
    join_status = GRankJoinGameAsync
      (ranked_player->context,
       s_rankings_game_id, SV_RankJoinGameCBF, cbf_arg, GR_OPT_END);
  ...
  if (ranked_player->final_status != QGR_STATUS_NEW)
  {
    ...
    cleanup_status = GRankCleanupAsync
      (ranked_player->context,
       0, SV_RankCleanupCBF, (void *) ranked_player, GR_OPT_END);
...
SV_RankJoinGameCBF(GR_JOINGAME * gr_joingame, void *cbf_arg)
{
  ...
  if (gr_joingame->status == GR_STATUS_OK)
  {
	...
	SV_RankUserValidate(i, NULL, NULL, 0, gr_joingame->rank,
			    ranked_player->name);
  }
  else
  {
    ...
    cleanup_status = GRankCleanupAsync
      (ranked_player->context, 0, SV_RankCleanupCBF, cbf_arg, GR_OPT_END);
...
SV_RankSendReportsCBF(GR_STATUS * status, void *cbf_arg)
{
  ...
  {
    cleanup_status = GRankCleanupAsync
      (context, 0, SV_RankCleanupCBF, cbf_arg, GR_OPT_END);
...
*/
static int
SV_RankAsciiEncode(char *dest, const unsigned char *src, int src_len)
{
  unsigned char bin[3];
  unsigned char txt[4];
  int dest_len = 0;
...
*/
static int
SV_RankAsciiDecode(unsigned char *dest, const char *src, int src_len)
{
...
*/
static void
SV_RankEncodeGameID(uint64_t game_id, char *result, int len)
{
  assert(result != NULL);
