...
CL_SystemInfoChanged(void)
{
  ...
  while (s)
  {
    ...
    else
    {
      ...
      if (!
	  (cvar_flags &
	   (CVAR_SYSTEMINFO | CVAR_SERVER_CREATED | CVAR_USER_CREATED)))
      {
#ifndef STANDALONE
	if (Q_stricmp(key, "g_synchronousClients")
	    && Q_stricmp(key, "pmove_fixed") && Q_stricmp(key, "pmove_msec"))
#endif
	{
	  Com_Printf(S_COLOR_YELLOW
...
CL_ParseServerInfo(void)
{
  ...
  Q_strncpyz(clc.sv_dlURL,
	     Info_ValueForKey(serverInfo, "sv_dlURL"), sizeof(clc.sv_dlURL));
}
