...
mumble_link(const char *name)
{
  ...
  lm =
    (LinkedMem *) MapViewOfFile(hMapObject, FILE_MAP_ALL_ACCESS, 0, 0,
				sizeof(LinkedMem));
  if (lm == NULL)
  {
  ...
  lm =
    (LinkedMem
     *) (mmap(NULL, sizeof(LinkedMem), PROT_READ | PROT_WRITE, MAP_SHARED,
	      shmfd, 0));
  if (lm == (void *) (-1))
  {
    lm = NULL;
...
void
mumble_update_coordinates(float fPosition[3], float fFront[3], float fTop[3])
{
...
void
mumble_update_coordinates2(float fAvatarPosition[3], float fAvatarFront[3],
...
void
mumble_set_identity(const char *identity)
{
  size_t len;
  if (!lm)
    return;
...
void
mumble_set_context(const unsigned char *context, size_t len)
{
  if (!lm)
    return;
...
void
mumble_set_description(const char *description)
{
  size_t len;
  if (!lm)
    return;
