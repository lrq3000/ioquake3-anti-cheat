...
S_AL_HearingThroughEntity(int entityNum)
{
  ...
  if (clc.clientNum == entityNum)
  {
    ...
    distanceSq = DistanceSquared(entityList[entityNum].origin,
				 lastListenerOrigin);
...
S_AL_SrcAlloc(alSrcPriority_t priority, int entnum, int channel)
{
  ...
  for (i = 0; i < srcCount; i++)
  {
    ...
    if ((curSource->entity == entnum) && (curSource->channel == channel)
	&& (channel != 0))
    {
      S_AL_SrcKill(i);
      return i;
...
S_AL_StartSound(vec3_t origin, int entnum, int entchannel, sfxHandle_t sfx)
{
  ...
  if ((srcActiveCnt > 5 * srcCount / 3) &&
      (DistanceSquared(sorigin, lastListenerOrigin) >=
       (s_alMaxDistance->value +
	s_alGraceDistance->value) * (s_alMaxDistance->value +
				     s_alGraceDistance->value)))
  {
...
*/
static void
S_AL_AddLoopingSound(int entityNum, const vec3_t origin,
	...
	if (!curSource->isPlaying)
	{
	  if (curSource->priority == SRCPRI_AMBIENT)
	  {
	    ...
	    if (curSfx->loopActiveCnt)
	    {
	      ...
	      S_AL_ClearError(qfalse);
	      qalGetSourcei(srcList[curSfx->masterLoopSrc].
			    alSource, AL_SAMPLE_OFFSET, &offset);
	      if ((error = qalGetError()) != AL_NO_ERROR)
	      {
		if (error != AL_INVALID_ENUM)
		{
	    ...
	    else if (curSfx->loopCnt && curSfx->masterLoopSrc >= 0)
	    {
	      ...
	      {
		secofs =
		  master->lastTimePos + (Sys_Milliseconds() -
					 master->lastSampleTime) / 1000.0f;
		secofs =
		  fmodf(secofs,
			(float) curSfx->info.samples / curSfx->info.rate);
	  ...
	  else if (curSource->lastTimePos >= 0)
	  {
	    ...
	    secofs =
	      curSource->lastTimePos + (Sys_Milliseconds() -
					curSource->lastSampleTime) / 1000.0f;
	    secofs =
	      fmodf(secofs, (float) curSfx->info.samples / curSfx->info.rate);
	    qalSourcef(curSource->alSource, AL_SEC_OFFSET, secofs);
	  }
...
*/
static void
S_AL_RawSamples(int stream, int samples, int rate, int width, int channels,
...
*/
static void
S_AL_Respatialize(int entityNum, const vec3_t origin, vec3_t axis[3],
		  int inwater)
{
  float orientation[6];
  vec3_t sorigin;
...
S_AL_Init(soundInterface_t * si)
{
  ...
  if (enumeration_ext || enumeration_all_ext)
  {
    ...
    s_alAvailableDevices =
      Cvar_Get("s_alAvailableDevices", devicenames,
  ...
  else
  {
	...
	Cvar_Get("s_alAvailableInputDevices", inputdevicenames,
      ...
      Com_Printf("OpenAL default capture device is '%s'\n",
		 defaultinputdevice);
      alCaptureDevice =
	qalcCaptureOpenDevice(inputdevice, 8000, AL_FORMAT_MONO16, 4096);
