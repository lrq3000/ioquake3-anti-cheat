...
SCR_AdjustFrom640(float *x, float *y, float *w, float *h)
{
  ...
  {
    *x +=
      0.5 * (cls.glconfig.vidWidth - (cls.glconfig.vidHeight * 640 / 480));
  }
#endif
...
*/
void
SCR_DrawStringExt(int x, int y, float size, const char *string,
...
SCR_DrawDemoRecording(void)
{
  ...
  SCR_DrawStringExt(320 - strlen(string) * 4, 20, 8, string,
...
SCR_DrawDebugGraph(void)
{
  ...
  {
    i =
      (ARRAY_LEN(values) + current - 1 -
       (a % ARRAY_LEN(values))) % ARRAY_LEN(values);
    v = values[i];
    v = v * cl_graphscale->integer + cl_graphshift->integer;

    if (v < 0)
      v +=
	cl_graphheight->integer * (1 + (int) (-v / cl_graphheight->integer));
