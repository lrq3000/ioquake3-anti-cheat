...
*/
void
S_SpatializeOrigin(vec3_t origin, int master_vol, int *left_vol,
		   int *right_vol)
{
  vec_t dot;
  vec_t dist;
...
*/
void
S_Base_AddLoopingSound(int entityNum, const vec3_t origin,
		       const vec3_t velocity, sfxHandle_t sfxHandle)
{
  ...
  if (s_doppler->integer && VectorLengthSquared(velocity) > 0.0)
  {
    ...
    lena =
      DistanceSquared(loopSounds[listener_number].origin,
...
*/
void
S_ByteSwapRawSamples(int samples, int width, int s_channels,
		     const byte * data)
{
  int i;
...
*/
void
S_Base_RawSamples(int stream, int samples, int rate, int width,
...
S_UpdateBackgroundTrack(void)
{
  ...
  while (s_rawend[0] < s_soundtime + MAX_RAW_SAMPLES)
  {
    ...
    // our max buffer size
    fileBytes =
      fileSamples * (s_backgroundStream->info.width *
		     s_backgroundStream->info.channels);
    if (fileBytes > sizeof(raw))
    {
      fileBytes = sizeof(raw);
      fileSamples =
	fileBytes / (s_backgroundStream->info.width *
		     s_backgroundStream->info.channels);
    }
    ...
    if (r < fileBytes)
    {
      fileBytes = r;
      fileSamples =
	r / (s_backgroundStream->info.width *
	     s_backgroundStream->info.channels);
    }
