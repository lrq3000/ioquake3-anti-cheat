  ...
  while (target)
  {
    ...
    if (val < 0 || val >= MAX_CLIENTS)
    {
      Com_Printf(S_COLOR_YELLOW "WARNING: VoIP "
		 "target %d is not a valid client " "number\n", val);
...
CL_CaptureVoip(void)
{
  ...
  {				// user wants to capture audio?
	...
	bytes = speex_bits_write(&clc.speexEncoderBits,
				 (char *) &clc.voipOutgoingData[wpos +
								1],
...
CL_RequestAuthorization(void)
{
  ...
  NET_OutOfBandPrint(NS_CLIENT, cls.authorizeServer, "getKeyAuthorize %i %s",
		     fs->integer, nums);
}
#endif
/*
...
*/
void
CL_ServersResponsePacket(const netadr_t * from, msg_t * msg,
			 qboolean extended)
{
  int i, j, count, total;
  ...
  if (!Q_stricmp(c, "challengeResponse"))
  {
	...
	if (com_legacyprotocol->integer > 0)
	{
	  ...
	  Com_Printf(S_COLOR_YELLOW
		     ...
		     com_protocol->integer, com_legacyprotocol->integer);
	}
	else
#endif
	{
...
CL_Init(void)
{
  ...
  cl_voip =
    Cvar_Get("cl_voip", "1", CVAR_USERINFO | CVAR_ARCHIVE | CVAR_LATCH);
...
CL_ServerStatusResponse(netadr_t from, msg_t * msg)
{
  ...
  Com_sprintf(&serverStatus->string[len],
	      sizeof(serverStatus->string) - len, "\\");
	  ...
	  if (source == AS_GLOBAL)
	  {
	    ...
	    if (cls.numGlobalServerAddresses > 0)
	    {
	      ...
	      CL_InitServerInfo(&server[i],
				&cls.globalServerAddresses[cls.
							   numGlobalServerAddresses]);
