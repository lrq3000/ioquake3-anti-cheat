 ...
 */
static void
PPC_Append(dest_instruction_t * di_now, unsigned long int i_count)
{
 ...
 */
static void
PPC_AppendInstructions(unsigned long int i_count,
		       size_t num_instructions, const ppc_instruction_t * is)
{
...
static void
PPC_PrepareJump(unsigned long int i_count,
 ...
 */
static void
VM_AnalyzeFunction(source_instruction_t * const i_first,
		   long int *prepareStack,
		   long int *gpr_start_pos, long int *fpr_start_pos)
{
  ...
  i_now = i_first;
  long int needed_reg[4] = { 0, 0, 0, 0 }, max_reg[4] =
  {
  0, 0, 0, 0};
  opstack_depth = 0;
  while ((i_now = i_now->next))
...
VM_CallCompiled(vm_t * vm, int *args)
{
  ...
  {
    printf
      ("App clock: %ld, vm total: %ld, vm this: %ld, vm real: %ld, vm out: %ld\n"
       "Inside VM %f%% of app time\n", stop_time.tms_utime, time_total_vm,
       time_diff, time_diff - time_outside_vm, time_outside_vm,
