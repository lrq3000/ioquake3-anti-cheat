...
*/
static void
CM_Subdivide(vec3_t a, vec3_t b, vec3_t c, vec3_t out1, vec3_t out2,
	     vec3_t out3)
{
  int i;
...
CM_SubdivideGridColumns(cGrid_t * grid)
{
  ...
  for (i = 0; i < grid->width - 2;)
  {
    ...
    for (j = 0; j < grid->height; j++)
    {
	  ...
	  (grid->points[i][j], grid->points[i + 1][j],
	   grid->points[i + 2][j]))
      {
	break;
      }
...
*/
static int
CM_EdgePlaneNum(cGrid_t * grid,
		int gridPlanes[MAX_GRID_SIZE][MAX_GRID_SIZE][2], int i,
		int j, int k)
{
  float *p1, *p2;
  vec3_t up;
...
CM_AddFacetBevels(facet_t * facet)
{
  ...
  for (j = 0; j < w->numpoints; j++)
  {
	...
	if (i == facet->numBorders)
	{
	  ...
	  //
	  w2 = CopyWinding(w);
	  Vector4Copy(planes[facet->borderPlanes[facet->numBorders]].
		      plane, newplane);
...
CM_TraceThroughPatchCollide(traceWork_t * tw, const struct patchCollide_s *pc)
{
  ...
  for (i = 0; i < pc->numFacets; i++, facet++)
  {
	...
	(plane, startp, endp, &enterFrac, &leaveFrac, &hit))
    {
      continue;
    }
    if (hit)
...
*/
qboolean
CM_PositionTestInPatchCollide(traceWork_t * tw,
			      const struct patchCollide_s *pc)
{
  int i, j;
  float offset, t;
...
void
CM_DrawDebugSurface(void (*drawPoly)
		    (int color, int numPoints, float *points))
{
  static cvar_t *cv;
#ifndef BSPC
  static cvar_t *cv2;
