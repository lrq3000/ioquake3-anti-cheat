...
static void
EmitRexString(byte rex, const char *string)
{
#if idx64
  if (rex)
    Emit1(rex);
...
static void
EmitCommand(ELastCommand command)
{
  switch (command)
  {
  ...
  if (!jlabel)
  {
    ...
    {				// mov [edi + ebx * 4], eax
      ...
      vm->instructionPointers[instruction - 1] = compiledOfs;
	     ...
	     && buf[compiledOfs - 6] == 0x04 && buf[compiledOfs - 5] == 0x9F)
  ...
  if (!jlabel)
  {
	     ...
	     || pop1 == OP_STORE1)
    {
      EmitString("8B D0");	// mov edx, eax
	     ...
	     && buf[compiledOfs - 6] == 0x07 && buf[compiledOfs - 5] == 0x9F)
...
void
DoSyscall(int syscallNum, int programStack, int *opStackBase,
	  uint8_t opStackOfs, intptr_t arg)
{
#else
static void
DoSyscall(void)
{
  ...
  mov dword ptr syscallNum, eax
      ...
      mov dword ptr opStackBase, edi mov dword ptr arg, ecx}
#endif
#else
  __asm__ volatile ("":"=a" (syscallNum), "=S"(programStack),
		    "=D"(opStackBase), "=b"(opStackOfs), "=c"(arg));
#endif
  ...
  else
  {
	...
	Com_Error(ERR_DROP, "VM_BLOCK_COPY failed due to corrupted opStack");
      ...
      VM_BlockCopy(opStackBase[(opStackOfs - 1)],
...
VM_Compile(vm_t * vm, vmHeader_t * header)
{
  ...
  for (pass = 0; pass < 3; pass++)
  {
	...
	if (buf[compiledOfs - 3] == 0x89 && buf[compiledOfs - 2] == 0x04
	...
	VMFREE_BUFFERS();
	Com_Error(ERR_DROP,
		  "VM_CompileX86: bad opcode %i at offset %i", op, pc);
      }
      pop0 = pop1;
      pop1 = op;
    }
...
#elif _WIN32
  {
    ...
    if (!VirtualProtect
	(vm->codeBase, compiledOfs, PAGE_EXECUTE_READ, &oldProtect))
...
VM_CallCompiled(vm_t * vm, int *args)
{
  ...
  *opStack = 0xDEADBEEF;
...
#if idx64
  opStackOfs =
    qvmcall64(&programStack, opStack, vm->instructionPointers, vm->dataBase);
#else
  __asm
  {
  pushad
      ...
      mov dword ptr opStack, edi mov dword ptr programStack, esi popad}
#endif
#elif idx64
  __asm__ volatile ("movq %5, %%rax\n"
		    ...
		    "+b"(opStackOfs):"g"(vm->instructionPointers),
		    "g"(vm->dataBase), "g"(entryPoint):"cc", "memory",
		    "%rax", "%rcx", "%rdx", "%r8", "%r9", "%r10", "%r11");
#else
  __asm__ volatile ("calll *%3\n":"+S" (programStack), "+D"(opStack),
		    "+b"(opStackOfs):"g"(entryPoint):"cc", "memory", "%eax",
		    "%ecx", "%edx");
#endif
