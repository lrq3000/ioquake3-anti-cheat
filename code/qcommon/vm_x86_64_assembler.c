...
static __attribute__ ((noreturn)) __attribute__ ((format(printf, 2, 3)))
     void _crap(const char *func, const char *fmt, ...)
{
  va_list ap;
...
static unsigned
hashkey(const char *string, unsigned len)
{
...
// no dup check!
static void
hash_add_label(const char *label, unsigned address)
{
  struct hashentry *h;
...
static void
emit_opsingle16(const char *mnemonic, arg_t arg1, arg_t arg2, void *data)
{
  emit1(0x66);
...
static void
compute_rexmodrmsib(u8 * rex_r, u8 * modrm_r, u8 * sib_r, arg_t * arg1,
		    arg_t * arg2)
{
  u8 rex = 0;
  u8 modrm = 0;
  ...
  if (arg2->type == T_MEMORY)
  {
    if ((arg2->v.mem.basetype == T_REGISTER
	 && !(arg2->v.mem.base.reg & R_64))
	|| (arg2->v.mem.indextype == T_REGISTER
	    && !(arg2->v.mem.index.reg & R_64)))
    {
      crap("only 64bit base/index registers are %x %x",
	   arg2->v.mem.base.reg, arg2->v.mem.index.reg);
    }
...
static void
maybe_emit_displacement(arg_t * arg)
{
...
static void
emit_op_rm(const char *mnemonic, arg_t arg1, arg_t arg2, void *data)
{
  u8 rex, modrm, sib;
  opparam_t *params = data;

  if ((arg1.type != T_REGISTER && arg1.type != T_MEMORY)
      || arg2.type != T_NONE)
    CRAP_INVALID_ARGS;
...
static void
emit_op_rm_cl(const char *mnemonic, arg_t arg1, arg_t arg2, void *data)
{
  u8 rex, modrm, sib;
  opparam_t *params = data;
...
emit_subaddand(const char *mnemonic, arg_t arg1, arg_t arg2, void *data)
{
  ...
  else if (arg1.type == T_REGISTER
	   && (arg2.type == T_MEMORY || arg2.type == T_REGISTER))
  {
    compute_rexmodrmsib(&rex, &modrm, &sib, &arg1, &arg2);
...
static void
emit_condjump(const char *mnemonic, arg_t arg1, arg_t arg2, void *data)
{
  unsigned off;
  int disp;
...
static void
emit_jmp(const char *mnemonic, arg_t arg1, arg_t arg2, void *data)
{
  if ((arg1.type != T_LABEL && arg1.type != T_REGISTER
       && arg1.type != T_MEMORY) || arg2.type != T_NONE)
    CRAP_INVALID_ARGS;
...
static void
emit_call(const char *mnemonic, arg_t arg1, arg_t arg2, void *data)
{
  u8 rex, modrm, sib;

  if ((arg1.type != T_REGISTER && arg1.type != T_IMMEDIATE)
      || arg2.type != T_NONE)
    CRAP_INVALID_ARGS;
...
static void
emit_twobyte(const char *mnemonic, arg_t arg1, arg_t arg2, void *data)
{
  u8 rex, modrm, sib;
...
nexttok(const char **str, char *label, u64 * val)
{
  const char *s = *str;
...
size_t
assembler_get_code_size(void)
{
  return compiledOfs;
}

void
assembler_set_output(char *buf)
{
  out = buf;
}

void
assemble_line(const char *input, size_t len)
{
  char line[4096];
  char *s;
  op_t *o;
