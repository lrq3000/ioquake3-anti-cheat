 ...
 */
static void
FS_CheckFilenameIsNotExecutable(const char *filename, const char *function)
{
...
{
  return
    FS_FileInPathExists(FS_BuildOSPath
			(fs_homepath->string, fs_gamedir, file));
}
...
FS_FOpenFileReadDir(const char *filename, searchpath_t * search,
		    fileHandle_t * file, qboolean uniqueFILE, qboolean unpure)
{
  long hash;
  pack_t *pak;
...
*/
long
FS_FOpenFileRead(const char *filename, fileHandle_t * file,
		 qboolean uniqueFILE)
{
  searchpath_t *search;
  long len;
  ...
  while (search)
  {
	...
	*startSearch = search;
	...
	if (!FS_FilenameCompare
	    (lastSearch->pack->pakPathname, pack->pakPathname))
	{
	  search = search->next;
	  continue;
	}
...
FS_LoadZipFile(const char *zipfile, const char *basename)
{
  ...
  if (strlen(pack->pakBasename) > 4
      && !Q_stricmp(pack->pakBasename + strlen(pack->pakBasename) - 4,
		    ".pk3"))
  {
  ...
  pack->checksum =
    Com_BlockChecksum(&fs_headerLongs[1],
		      sizeof(*fs_headerLongs) * (fs_numHeaderLongs - 1));
  pack->pure_checksum =
    Com_BlockChecksum(fs_headerLongs,
		     ...
		     int *numfiles, qboolean allowNonPureFilesOnDisk)
{
  ...
  for (search = fs_searchpaths; search; search = search->next)
  {
	...
	continue;
      ...
      {
	netpath =
	  FS_BuildOSPath(search->dir->path, search->dir->gamedir, path);
	sysFiles =
	  Sys_ListFiles(netpath, extension, filter, &numSysFiles, qfalse);
...
FS_Startup(const char *gameName)
{
  ...
  fs_basepath =
    Cvar_Get("fs_basepath", Sys_DefaultInstallPath(),
...
FS_CheckPak0(void)
{
  ...
  for (path = fs_searchpaths; path; path = path->next)
  {
	...
	founddemo = qtrue;
	     ...
	     && !Q_stricmpn(pakBasename, "pak", 3) && pakBasename[3] >= '0'
	     && pakBasename[3] <= '0' + NUM_ID_PAKS - 1)
    {
...
FS_InitFilesystem(void)
{
  ...
  if (!FS_FilenameCompare
      (Cvar_VariableString("fs_game"), com_basegame->string))
    Cvar_Set("fs_game", "");
