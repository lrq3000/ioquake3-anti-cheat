...
//              iargs[i+1] = *(int *)((byte *)currentVM->dataBase + callProgramStack + 8 + 4*i);
    args[i + 1] =
      *(int *) ((byte *) currentVM->dataBase + callProgramStack + 8 + 4 * i);
  }
...
static __attribute__ ((noreturn))
     void CROSSCALL jmpviolation(void)
{
...
#ifdef DEBUG_VM
static __attribute__ ((noreturn))
     void CROSSCALL memviolation(void)
{
  Com_Error(ERR_DROP,
...
static __attribute__ ((noreturn))
     void CROSSCALL opstackviolation(void)
{
...
VM_Compile(vm_t * vm, vmHeader_t * header)
{
  ...
  for (pass = 0; pass < 2; ++pass)
  {
    ...
    if (pass)
    {
      ...
      vm->codeBase =
	mmap(NULL, compiledOfs, PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS,
	     -1, 0);
	...
	Com_Error(ERR_FATAL, "VM_CompileX86_64: can't mmap memory");
      ...
      vm->codeBase =
	VirtualAlloc(NULL, compiledOfs, MEM_RESERVE | MEM_COMMIT,
		     PAGE_READWRITE);
      if (!vm->codeBase)
	...
	if (got_const)
	{
	  if ((int) const_value >= 0)
	  {
	    ...
	    emit("movq $%" PRIu64 ", %%rax",
		 vm->codeBase + vm->instructionPointers[const_value]);
	    emit("callq *%%rax");
	    got_const = 0;
	    break;
	  }
