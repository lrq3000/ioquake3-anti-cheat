...
static unsigned int
sparc_assemble(enum sparc_iname iname, const int argc, const int *argv)
{
...
#if 0
static void
pgreg(int reg_num, int arg_index, int flt)
{
  if (!flt)
  {
    ...
    Com_Printf("%s%s%d",
	       (arg_index ? ", " : ""), fmt[reg_num >> 3], reg_num & 7);
  }
  else
...
static void
sparc_disassemble(unsigned int insn)
{
  int op_idx;
...
static unsigned int
sparc_push_data(struct func_info *const fp, unsigned int val)
{
...
static void
dst_insn_insert_tail(struct func_info *const fp, struct dst_insn *dp)
{
  if (!fp->dst_first)
  {
...
static void
jump_insn_insert_tail(struct func_info *const fp, struct jump_insn *jp)
{
  if (!fp->jump_first)
  {
...
static void
dst_insn_append(struct func_info *const fp)
{
...
static void
jump_insn_append(vm_t * vm, struct func_info *const fp,
...
static void
__do_emit_one(struct func_info *const fp, unsigned int insn)
{
...
static void
end_emit(struct func_info *const fp)
{
...
static void
blockcopy(unsigned int dest, unsigned int src, unsigned int count)
{
...
static void
do_emit_const(struct func_info *const fp, struct src_insn *sp)
{
...
static void
free_source_insns(struct func_info *const fp)
{
...
sparc_compute_code(vm_t * vm, struct func_info *const fp)
{
...
#if 0
  {
	...
	("\t.word\t0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
	 insn[i + 0], insn[i + 1], insn[i + 2], insn[i + 3], insn[i + 4],
	 insn[i + 5], insn[i + 6], insn[i + 7]);
    }
  }
#endif
