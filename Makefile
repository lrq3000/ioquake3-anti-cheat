...
ifeq ($(PLATFORM),mingw32)
  ...
  ifeq ($(USE_OPENAL),1)
    ...
    CLIENT_LIBS += $(LIBSDIR)/win32/OpenAL32.lib
    ifneq ($(USE_OPENAL_DLOPEN),1)
...
#############################################################################
## PAK ARCHIVE (QVM)
## NOTE: NO MISSIONPACK, ONLY FOR STANDARD GAME
#############################################################################
...
# _ML_
ifneq ($(BUILD_PAK),0)
	$(COPY) -f $(BR)/pak$(PAK_NUM).pk3 $(COPYDIR)/$(BASEGAME)
endif
# _ML_
